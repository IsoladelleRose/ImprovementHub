...
# Find and copy the JAR file with a generic name
RUN find target -name "*.jar" -type f -exec cp {} /app/backend.jar \;
...
COPY --from=backend-build /app/backend.jar /app/backend.jar
